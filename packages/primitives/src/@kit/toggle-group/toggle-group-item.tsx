...
import type { Direction, Item, Orientation } from './toggle-group-context';
import { toggleGroupRootApiContextId } from './toggle-group-context';

type NavigationKeys = KeyCode.ArrowRight | KeyCode.ArrowLeft | KeyCode.ArrowDown | KeyCode.ArrowUp;

type Step = -1 | 0 | 1;

const keyNavigationMap: Record<Orientation, Record<Direction, Record<NavigationKeys, Step>>> = {
  horizontal: {
    ltr: {
      ArrowRight: 1,
      ArrowLeft: -1,
...
export type HToggleGroupItemProps = PropsOf<typeof Toggle> & {
  value: string;
};

export const HToggleGroupItem = component$<HToggleGroupItemProps>((props) => {
  ...
  const handleKeyDown$ = $(async (event: KeyboardEvent) => {
    ...
    const items = [
      ...(document.querySelectorAll(`.toggle-group-item-${rootApiContext.rootId}`) as any),
    ] as HTMLElement[];
