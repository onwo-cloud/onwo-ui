...
export function useCombobox() {
  ...
  const filterManager$ = $(async (isVisible: boolean, itemRef: Signal, index: number) => {
    ...
    context.filteredIndexSetSig.value = new Set(
      isVisible
        ? [...context.filteredIndexSetSig.value].filter((filteredIndex) => filteredIndex !== index)
        : [...context.filteredIndexSetSig.value, index],
    );

    if (isDisabled) {
      context.disabledIndexSetSig.value = new Set([...context.disabledIndexSetSig.value, index]);
    } else {
      context.disabledIndexSetSig.value = new Set(
        [...context.disabledIndexSetSig.value].filter((disabledIndex) => disabledIndex !== index),
      );
    }
    ...
    if (context.filter === false) {
      ...
      const firstVisibleIndex = Array.from(
        { length: context.itemsMapSig.value.size },
        (_, idx) => idx,
