...
export function useSelect() {
  ...
  const selectionManager$ = $(async (index: number | null, action: 'add' | 'toggle' | 'remove') => {
    if (index === null) return;

    if (action === 'add') {
      if (context.multiple) {
        context.selectedIndexSetSig.value = new Set([...context.selectedIndexSetSig.value, index]);
      } else {
    ...
    if (action === 'toggle') {
      if (context.selectedIndexSetSig.value.has(index)) {
        context.selectedIndexSetSig.value = new Set(
          [...context.selectedIndexSetSig.value].filter((selectedIndex) => selectedIndex !== index),
        );
      } else {
        context.selectedIndexSetSig.value = new Set([...context.selectedIndexSetSig.value, index]);
      }
...
export function useTypeahead() {
  ...
  const indexDiffSig = useSignal<number | undefined>();
  const prevTimeoutSig = useSignal<undefined | TimeoutId>();
  const { selectionManager$ } = useSelect();

  const firstCharItemSig = useComputed$(() => {
    return [...context.itemsMapSig.value.values()].map((item) =>
  ...
  const typeahead$ = $(async (key: string): Promise<void> => {
    ...
    const multipleChars$ = $(async () => {
      ...
      for (const [index, item] of context.itemsMapSig.value) {
        const optStr = item.displayValue?.toLowerCase().slice(0, Math.max(0, inputSize));
