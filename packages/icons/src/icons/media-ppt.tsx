import { primitives as P } from '@onwo/ui';

export default (props: P.IconProps) => (
  <P.SvgIcon data--icon-name="media-ppt" {...props} viewBox="0 0 32 32">
    <path
      d="M14.1885 5.01034V9.79128C14.1885 10.9688 13.259 11.9241 12.1113 11.9241H7.0252M21.5382 27H10.4618C8.55016 27 7 25.4083 7 23.4453V12.3021C7 11.5166 7.3168 10.7658 7.8748 10.2283L12.4202 5.76926C12.9335 5.27463 13.611 5 14.3145 5H21.5382C23.4498 5 25 6.59168 25 8.5547V23.4453C25 25.4083 23.4506 27 21.5382 27Z"
      stroke="currentColor"
      stroke-linejoin="round"
    />
    <path
      d="M10 20V16H11.5141C12.3719 16 12.7447 16.568 12.7447 17.0793C12.7447 17.5907 12.3726 18.1587 11.5141 18.1587H10M14.4765 20V16H15.9906C16.8491 16 17.2212 16.568 17.2212 17.0793C17.2212 17.5907 16.8491 18.1587 15.9906 18.1587H14.4765M20.197 20V16.1113M22 16H18.3948"
      stroke="currentColor"
      stroke-linecap="round"
      stroke-linejoin="round"
    />
  </P.SvgIcon>
);
