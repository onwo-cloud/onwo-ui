...
import type { InternalComboboxProps } from './combobox-inline-comp';
 ...
 *
 */
type TMultiValue = { multiple: true; value?: never } | { multiple?: false; value?: string };
...
>((props: HComboboxRootImplProps<boolean> & InternalComboboxProps) => {
  ...
  const selectedValueSetSig = useSignal<Set<string>>(new Set(initialValue ? [initialValue] : []));
  ...
  const isNoItemsSig = useComputed$(() => {
    return (
      itemsMapSig.value.size === filteredIndexSetSig.value.size || itemsMapSig.value.size === 0
    );
  });
