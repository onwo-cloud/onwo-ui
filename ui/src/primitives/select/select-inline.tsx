  ...
  while (childrenToProcess.length > 0) {
    ...
    switch (child.type) {
      ...
      case SelectItemLabel: {
        ...
        const value = (givenItemValue === null ? displayValue : givenItemValue) as string;
        ...
        const isString =
          typeof child.props.children === 'string' || typeof child.props.children === 'object';

        if (!isString) {
          throw new Error(
            `Qwik UI: select item label passed was not a string. It was a ${typeof child.props
              .children}.`,
          );
        }
