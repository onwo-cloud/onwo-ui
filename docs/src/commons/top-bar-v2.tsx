...
export const StickyTopBar = component$(() => {
  ...
  useVisibleTask$(({ track }) => {
    ...
    track(() => linkRef.value);
    if (!stickyBarRef.value || !linkRef.value) return;
    ...
    scroll(
      (animate as any)(
        stickyBarRef.value,
        {
          width: [`${linkWidth + 48}px`, `${linkWidth + 48 + 24}px`],
        },
  ...
  return (
    <div
      ref={stickyBarRef}
      class={cn(
        'fixed left-1/2 -translate-x-1/2 flex px-6 py-2 bg-papyrus text-lead justify-center rounded-full items-center gap-6 transition-all duration-200',
        scrollBreakpoint.value && 'bg-black text-contrast',
      )}
    >
      {/* Logo that appears on scroll */}
      <div
        class={`absolute left-4 transition-all duration-300 ${scrollBreakpoint.value ? 'opacity-100' : 'opacity-0'}`}
      >
        <Logo size={20} />
      </div>
    ...
    >
      ...
      <div class="flex items-center gap-2">
        ...
        <div class="flex gap-6">
          <Link target="_blank" href="https://github.com/onwo-cloud/onwo-ui">
            <GithubIcon />
          </Link>
          {/*<ThemeIcon />*/}
        </div>
