...
export const SearchBar = component$<SearchBarProps>(
  ({
    ...
    return (
      <div
        class={`flex items-center gap-2 relative w-80 pl-4 pr-4 py-2 focus-within:ring-[blue] focus-within:ring-2 hover:ring-[blue] focus-within:hover:ring-[blue] ring-1 rounded-xl ${disabled ? 'opacity-60 cursor-not-allowed' : ''} ${className}`}
        role="search"
        aria-label="Search"
        onClick$={() => {
          inputRef.value?.focus();
      ...
      >
        {showIcon && (
          <Icon name="search"  size="sm" class="pointer-events-none flex-shrink-0" aria-hidden="true"  />
        )}
        ...
        {content.value.trim().length > 0 && (
          <button
            ...
            type="button"
          >
            <Icon name="x" size="xs" class="text-blue-500" />
          </button>
        )}
