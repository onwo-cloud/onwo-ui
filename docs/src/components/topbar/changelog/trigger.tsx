...
export const ChangelogTrigger = component$(() => {
  ...
  // Track if the user has toggled the state to prevent mount animation
  ...
  return (
    <div
      class={[
        ...
        !ctx.isExpanded.value && "flex items-center justify-between bg-transparent rounded-[19.5px]"
      ]}
    >
      <style>{`
        @keyframes blurIn {
      ...
      {/* 
        Keep the Link and Separator statically in the DOM. 
        Toggling visibility with display classes prevents Qwik hydration/SSG mismatches.
      */}
      <Link
        href={latest.href}
        class={[
      ...
      >
        ...
        >
          <div class={["flex items-center gap-2", ctx.isExpanded.value && animationClass]}>
