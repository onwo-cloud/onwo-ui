...
const MobileMenuTabsRoot = component$((props: { value: Signal<TabValue> }) => {
  ...
  return (
    <div class="relative flex items-center rounded-full flex-1 p-0.75 [box-shadow:#0000000A_0px_7px_15px,#0000000D_0px_2px_2px,#0000000A_0px_2px_4px] bg-shade-0 [outline:1px_solid_#0000000A] overflow-hidden">
      ...
      <div
        ref={overlayRef}
        class="absolute inset-[3px] z-10 overflow-hidden rounded-full bg-shade-0 hidden"
        style={{ display: 'none' }}
      >
        {/* Layer 1: Grey Cover background (slides in 30% behind the elements) */}
        <div
          ref={coverRef}
          class="absolute inset-0 rounded-full bg-shade-150 h-full w-full"
        ...
        >
          ...
          <input
            ref={inputRef}
            type="text"
            class="bg-transparent border-none outline-none font-['Geist-Regular','Geist',system-ui,sans-serif] text-shade-900 placeholder:text-shade-600 text-[16px] leading-none flex-1 w-full h-full min-w-0 pointer-events-auto"
            placeholder="Search"
            style={{ opacity: 0 }}
          />
        </div>
  ...
  return (
    <Button
      ...
      class={[
        ...
        isActive ? 'bg-shade-150' : 'bg-transparent',
      ]}
    >
      <div
        ...
        class={[
          ...
          isActive
            ? 'text-shade-950 [&_.fill-target]:fill-shade-950'
            : 'text-shade-800 [&_.fill-target]:fill-none',
        ]}
      >
        {props.icon}
      </div>
      <div
        class={[
          ...
          props.value === 'search' && isSearchActive ? 'opacity-0' : 'opacity-100',
          isActive ? 'text-shade-1000' : 'text-shade-800',
        ]}
      >
        {props.name}
      </div>
...
const MobileMenuActionButton = component$((props: OwPropsOf<'div'>) => (
  <Button
    as="div"
    class="flex items-start gap-6.5 rounded-full p-4 bg-shade-900 cursor-pointer border-none outline-none shrink-0"
    {...props}
  >
    <Slot />
  </Button>
...
export const MobileMenu = component$((props: OwPropsOf<typeof MobileMenuRoot>) => {
  ...
  return (
    <MobileMenuRoot {...props}>
      ...
      >
        <div class="relative w-6 h-6 flex items-center justify-center text-shade-0">
          {/* Ellipsis */}
          <div
            class={[
